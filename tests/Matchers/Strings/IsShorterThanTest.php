...
use LogicException;
use Mokkd\Matchers\Strings\IsShorterThan;
...
class IsShorterThanTest extends TestCase
{
    ...
    public function testConstructor1(int $length): void
    {
        ...
        new IsShorterThan($length);
    }

    /** Ensure encoding is UTF-8 by default. */
    public function testConstructor2(): void
    {
        self::assertSame("UTF-8", (new IsShorterThan(10))->encoding());
    }
    ...
    {
        self::assertSame($length, (new IsShorterThan($length))->length());
    }
    ...
    {
        self::assertSame($encoding, (new IsShorterThan(10, $encoding))->encoding());
    }
    ...
    {
        self::assertTrue((new IsShorterThan($length))->matches($string));
    }
    ...
    {
        self::assertFalse((new IsShorterThan($length))->matches($string));
    }
    ...
    public function testMatches3(int $length, string $encoding, string $encodedString): void
    {
        ...
        self::assertTrue((new IsShorterThan($length, $encoding))->matches($encodedString));
    }
    ...
    {
        self::assertSame($expected, (new IsShorterThan($length, $encoding))->matches($encodedString));
    }
    ...
    {
        self::assertSame("({$encoding}-string[<{$length}])", (new IsShorterThan($length, $encoding))->describe(self::nullSerialiser()));
    }

    /** Ensure the matcher describes itself with UTF-8 encoding by default. */
    public static function testDescribe2(): void
    {
        self::assertSame("(UTF-8-string[<10])", (new IsShorterThan(10))->describe(self::nullSerialiser()));
    }
}
