...
use LogicException;
use Mokkd\Matchers\Strings\IsStringNoShorterThan;
...
/**
 * Tests IsStringNoShorterThan matcher with various character encodings. Of those that the mbstring extension supports,
 ...
 */
class IsStringNoShorterThanTest extends TestCase
{
    ...
    public function testConstructor1(int $length): void
    {
        ...
        new IsStringNoShorterThan($length);
    }

    /** Ensure encoding is UTF-8 by default. */
    public function testConstructor2(): void
    {
        self::assertSame("UTF-8", (new IsStringNoShorterThan(10))->encoding());
    }
    ...
    {
        self::assertSame($length, (new IsStringNoShorterThan($length))->length());
    }
    ...
    {
        self::assertSame($encoding, (new IsStringNoShorterThan(10, $encoding))->encoding());
    }

    public static function dataForTestMatches1(): iterable
    {
        yield "string-no-shorter-than-zero-empty" => [0, ""];
        yield "string-no-shorter-than-zero-character" => [0, "a"];
        yield "string-no-shorter-than-zero-string" => [0, "mokkd"];

        foreach (DataFactory::positiveIntegers() as $length) {
            $length = DataFactory::unboxSingle($length);
            yield "string-no-shorter-than-{$length}-string-equal-length" => [$length, str_repeat("m", $length)];
            yield "string-no-shorter-than-{$length}-string-plus-one" => [$length, str_repeat("m", $length + 1)];
            yield "string-no-shorter-than-{$length}-string-plus-10" => [$length, str_repeat("k", $length + 10)];
            yield "string-no-shorter-than-{$length}-multibyte-utf8-characters" => [$length, str_repeat("\xc3\xa9", $length + 10)];
        }
    }
    ...
    {
        self::assertTrue((new IsStringNoShorterThan($length))->matches($string));
    }
    ...
    public static function dataForTestMatches2(): iterable
    {
        foreach (DataFactory::positiveIntegers() as $length) {
            $length = DataFactory::unboxSingle($length);
            yield "string-no-shorter-than-{$length}-string-minus-one" => [$length, str_repeat("m", $length - 1)];
            yield "string-no-shorter-than-{$length}-empty" => [$length, ""];
        }
    }

    /** Ensure a reasonable subset of shorter strings fail to match. */
    #[DataProvider("dataForTestMatches2")]
    public function testMatches2(int $length, string $string): void
    {
        self::assertFalse((new IsStringNoShorterThan($length))->matches($string));
    }

    public static function dataForTestMatches3(): iterable
    {
        // U+00e9 (é)
        yield "string-no-shorter-than-10-more-bytes-fewer-characters-u+00e9-utf8" => [10, "UTF-8", str_repeat("\xc3\xa9", 6)];
        yield "string-no-shorter-than-10-more-bytes-fewer-characters-u+00e9-utf16-ordered" => [10, "UTF-16", str_repeat("\x00\xe9", 6)];
        yield "string-no-shorter-than-10-more-bytes-fewer-characters-u+00e9-utf16le" => [10, "UTF-16LE", str_repeat("\xe9\x00", 6)];
        yield "string-no-shorter-than-10-more-bytes-fewer-characters-u+00e9-utf16be" => [10, "UTF-16BE", str_repeat("\x00\xe9", 6)];
        yield "string-no-shorter-than-10-more-bytes-fewer-characters-u+00e9-utf32le" => [10, "UTF-32LE", str_repeat("\x00\x00\xe9\x00", 6)];
        yield "string-no-shorter-than-10-more-bytes-fewer-characters-u+00e9-utf32be" => [10, "UTF-32BE", str_repeat("\x00\x00\x00\xe9", 6)];

        // U+10437 (𐐷)
        yield "string-no-shorter-than-6-more-bytes-fewer-characters-u+10437-utf8" => [10, "UTF-8", str_repeat("\xf0\x90\x90\xb7", 3)];
        yield "string-no-shorter-than-10-more-bytes-fewer-characters-u+10437-utf16-ordered" => [10, "UTF-16", str_repeat("\xd8\x01\xdc\x37", 3)];
        yield "string-no-shorter-than-10-more-bytes-fewer-characters-u+10437-utf16le" => [10, "UTF-16LE", str_repeat("\x01\xd8\x37\xdc", 3)];
        yield "string-no-shorter-than-10-more-bytes-fewer-characters-u+10437-utf16be" => [10, "UTF-16BE", str_repeat("\xd8\x01\xdc\x37", 3)];
        yield "string-no-shorter-than-10-more-bytes-fewer-characters-u+10437-utf31le" => [10, "UTF-32LE", str_repeat("\x01\x00\x37\x04", 3)];
        yield "string-no-shorter-than-10-more-bytes-fewer-characters-u+10437-utf31be" => [10, "UTF-32BE", str_repeat("\x00\x01\x04\x37", 3)];
    }

    /** Ensure strings of more than length bytes but fewer than length characters in the required encoding don't match */
    ...
    public function testMatches3(int $length, string $encoding, string $encodedString): void
    {
        ...
        self::assertFalse((new IsStringNoShorterThan($length, $encoding))->matches($encodedString));
    }
    ...
    public static function dataForTestMatches4(): iterable
    {
        foreach (self::SingleByteEncodings as $label => $encoding) {
            ...
            yield "string-no-shorter-than-9-same-{$label}" => [9, $encoding, $value, true];
            yield "string-no-shorter-than-8-longer-{$label}" => [8, $encoding, $value, true];
            yield "string-no-shorter-than-10-shorter-{$label}" => [10, $encoding, $value, false];
        }
    }
    ...
    {
        self::assertSame($expected, (new IsStringNoShorterThan($length, $encoding))->matches($encodedString));
    }
    ...
    {
        yield from DataFactory::relabel(DataFactory::matrix(self::Lengths, [...self::SingleByteEncodings, ...self::MultiByteEncodings]), "string-no-shorter-than-", RelabelMode::Prefix);
    }
    ...
    {
        self::assertSame("({$encoding}-string[>={$length}])", (new IsStringNoShorterThan($length, $encoding))->describe(self::nullSerialiser()));
    }

    /** Ensure the matcher describes itself with UTF-8 encoding by default. */
    public static function testDescribe2(): void
    {
        self::assertSame("(UTF-8-string[>=10])", (new IsStringNoShorterThan(10))->describe(self::nullSerialiser()));
    }
}
