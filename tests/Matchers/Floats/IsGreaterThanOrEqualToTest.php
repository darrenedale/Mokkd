...
use Mokkd\Matchers\Floats\IsGreaterThanOrEqualTo;
...
#[CoversClass(IsGreaterThanOrEqualTo::class)]
class IsGreaterThanOrEqualToTest extends TestCase
{
    ...
    /** Ensure floats greater than the constraint value by a small quantity match successfully. */
    #[DataProvider("dataForTestMatches1")]
    public function testMatches1(float $constraint, float $testValue): void
    {
        self::assertTrue((new IsGreaterThanOrEqualTo($constraint))->matches($testValue));
    }
    ...
    /** Ensure floats greater than the constraint value by a large quantity match successfully. */
    ...
    /** Ensure floats less than the constraint value by a small quantity don't match. */
    #[DataProvider("dataForTestMatches3")]
    public function testMatches3(float $constraint, float $testValue): void
    {
        self::assertFalse((new IsGreaterThanOrEqualTo($constraint))->matches($testValue));
    }
    ...
    /** Ensure floats less than the constraint value by a large quantity don't match. */
    ...
    {
        self::assertTrue((new IsGreaterThanOrEqualTo($constraint))->matches($constraint));
    }
    ...
    {
        self::assertFalse((new IsGreaterThanOrEqualTo($constraint))->matches($value));
    }
    ...
    {
        self::assertSame($expected, (new IsGreaterThanOrEqualTo($constraint))->describe(self::nullSerialiser()));
    }
}
