...
use Mokkd\Matchers\Integers\IsWithin;
...
class IsWithinTest extends TestCase
{
    ...
    /** Ensure ints on the bounds don't match. */
    ...
    /** Ensure floats within the int bounds don't match. */
    ...
    public function testMatches7(int $lowerBound, int $upperBound, float $testValue): void
    {
        ...
        $matcher = new IsWithin($lowerBound, $upperBound);
        self::assertTrue($matcher->matches((int) $testValue), "Integers\\IsWithin({$lowerBound}, {$upperBound}) does not match (int) {$testValue} (" . ((int) $testValue) . ")");

        // then prove that the float does not match
